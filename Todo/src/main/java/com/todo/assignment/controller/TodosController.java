...
public class TodosController {
	...
	void delete(@PathVariable int taskId) {
		...
		ts.deleteTodo(taskId);
	}
	@PreAuthorize("hasAuthority('ROLE_USER')")
	@PutMapping("/user/change/{todoId}")
	void markCompleted(@PathVariable int todoId,@RequestBody Todo todo) {
		ts.updateCompleted(todoId,todo);
	}
	@PreAuthorize("hasAuthority('ROLE_USER')")
	@GetMapping("/user/getTodo/{todoId}")
	Todo get(@PathVariable int todoId) {
		return ts.getTodo(todoId);
	}
}
